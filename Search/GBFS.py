...
class GreedyFrontier():
    def __init__(self):
        self.frontier = []
        self.count = 0  # To break ties in case of equal heuristics
    
    def add(self, node):
        heapq.heappush(self.frontier, (node.heuristic, self.count, node))
        self.count += 1
...
class Maze():
    ...
    def solve(self):
        ...
        start_heuristic = self.manhattan_distance(self.start, self.goal)
        start = Node(state=self.start, parent=None, action=None, heuristic=start_heuristic)
        ...
        while True:
            ...
            # Choose the node with lowest heuristic from the frontier
            ...
            for action, state in self.neighbors(node.state):
                if not frontier.contains_state(state) and state not in self.explored:
                    heuristic = self.manhattan_distance(state, self.goal)
